// https://github.com/expo/expo/blob/main/packages/%40expo/cli/src/utils/errors.ts

import chalk from "chalk";
import { AssertionError } from "node:assert";
import { execSync } from "node:child_process";

import { exit, exception, warn } from "./log.ts";
...
export class SilentError extends CommandError {
  constructor(messageOrError?: string | Error) {
    ...
    if (typeof messageOrError !== "string") {
      // forward the props of the incoming error for tests or processes outside
      // of expo-cli that use expo cli internals.
