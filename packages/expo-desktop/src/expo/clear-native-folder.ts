...
import { directoryExistsAsync } from "./utils/dir.ts";
import { isInteractive } from "./utils/interactive.ts";
import { logNewSection } from "./utils/ora.ts";
import { confirmAsync } from "./utils/prompts.ts";
...
/** Returns `true` if a certain subset of required iOS project files are intact. */
export async function hasRequiredMacOSFilesAsync(projectRoot: string) {
  try {
    // If any of the following required files are missing, then the project is malformed.
    await Promise.all([
      MacOSConfig.Paths.getAllXcodeProjectPaths(projectRoot),
      MacOSConfig.Paths.getAllPBXProjectPaths(projectRoot),
    ]);
    return true;
  } catch {
    return false;
...
export async function getMalformedNativeProjectsAsync(
  ...
  const VERIFIERS: Record<ArbitraryPlatform, (root: string) => Promise<boolean>> = {
    ...
    macos: hasRequiredMacOSFilesAsync,
  };
  ...
  return (
    await Promise.all(
      checkPlatforms.map(async (platform: string) => {
...
export async function maybeBailOnNativeModuleAsync(projectRoot: string) {
  ...
  try {
    // @ts-ignore Treat this as an optional dep - trying to avoid depending on expo
    const { isNativeModuleAsync } = await import("expo/internal/unstable-autolinking-exports");
